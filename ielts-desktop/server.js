...
// ---------- 初始数据：单词 ----------
{
  ...
  if (count === 0) {
    const initialWords = [
      ...
      ['administer', '管理，执行', 'He helps administer the school programs.', '学术'],
...
// ---------- 初始数据：真题 ----------
{
  ...
  if (count === 0) {
    const initialQuestions = [
      ...
      { module: 'reading', type: 'choice', prompt: 'Based on the article, technology has made communication:', options: JSON.stringify(['slower', 'more expensive', 'faster and easier', 'less important']), answer: '2', explanation: '科技使通讯更快更方便', difficulty: 1 },
...
app.post('/api/tasks', (req, res) => {
  ...
  const info = db.prepare('INSERT INTO tasks (creator_id, title, description, module, config) VALUES (?, ?, ?, ?, ?)').run(null, title, description || '', module, configStr);
  return res.json({ success: true, id: info.lastInsertRowid });
});
  ...
  const tx = db.transaction((arr) => {
    for (const q of arr) {
      if (q && q.prompt && q.answer !== undefined && q.module && q.type) {
        ...
        stmt.run(q.module, q.type, q.prompt, opts, ans, q.explanation || '', typeof q.difficulty === 'number' ? q.difficulty : 2);
      }
    }
  });
  try {
...
app.post('/api/ai/generate-words', async (req, res) => {
  ...
  const prompt = '请生成' + count + '个雅思' + category + '类单词，严格返回 JSON 数组。格式：[{"word":"...","meaning":"...","example":"..."}]';
  const result = await callAI(cfg, prompt);
  if (!result.success) return res.status(500).json({ success: false, message: result.content });
  try {
    const parsed = result.content.replace(/```json|```/g, '').trim();
    let arr = JSON.parse(parsed);
...
app.get('/api/admin/users', (req, res) => {
  const session = requireAdmin(req, res);
  if (!session) return;
  const users = db.prepare('SELECT u.id, u.username, u.role, u.created_at, (SELECT COUNT(*) FROM user_words uw WHERE uw.username = u.username AND uw.known = 1) AS known_words, (SELECT COUNT(*) FROM user_progress up WHERE up.username = u.username) AS answered, (SELECT COUNT(*) FROM daily_checkins dc WHERE dc.username = u.username) AS checkins, (SELECT COUNT(*) FROM study_sessions ss WHERE ss.username = u.username) AS sessions FROM users u ORDER BY u.created_at DESC').all();
...
app.get('/api/admin/users/:username/detail', (req, res) => {
  ...
  const knownWords = db.prepare('SELECT uw.id, uw.word_id, uw.known, uw.reviewed_at, w.word, w.meaning, w.category FROM user_words uw LEFT JOIN words w ON w.id = uw.word_id WHERE uw.username = ? AND uw.known = 1 ORDER BY uw.reviewed_at DESC').all(username);
  const progress = db.prepare("SELECT up.id, up.module, up.item_id, up.correct, up.created_at, CASE up.module WHEN 'question' THEN q.prompt WHEN 'speaking' THEN s.topic WHEN 'writing' THEN wt.prompt ELSE '' END AS title FROM user_progress up LEFT JOIN questions q ON up.module = 'question' AND q.id = up.item_id LEFT JOIN speaking_topics s ON up.module = 'speaking' AND s.id = up.item_id LEFT JOIN writing_topics wt ON up.module = 'writing' AND wt.id = up.item_id WHERE up.username = ? ORDER BY up.created_at DESC LIMIT 50").all(username);
  const checkins = db.prepare('SELECT date FROM daily_checkins WHERE username = ? ORDER BY date DESC LIMIT 30').all(username);
  const stats = db.prepare('SELECT (SELECT COUNT(*) FROM user_words WHERE username = ? AND known = 1) AS total_known, (SELECT COUNT(*) FROM user_progress WHERE username = ?) AS total_progress, (SELECT COUNT(*) FROM daily_checkins WHERE username = ?) AS total_checkins').get(username, username, username);
...
app.post('/api/admin/quizzes/generate', async (req, res) => {
  ...
  if (mode === 'ai') {
    ...
    if (cfg && cfg.api_key) {
      ...
      const prompt = '为学生"' + displayName + '"生成一份雅思练习卷。请严格返回 JSON。格式：{"words": [{"word":"...","meaning":"...","example":"..."}], "questions": [{"type":"choice","prompt":"题目","options":["A","B","C","D"],"answer":"A","explanation":"解析"}]}';
  ...
  const config = JSON.stringify({ mode, wordCount, questionCount });
...
app.get('/api/my-quizzes', (req, res) => {
  const session = requireLogin(req, res);
  if (!session) return;
  const quizzes = db.prepare("SELECT q.*, (SELECT COUNT(*) FROM quiz_items WHERE quiz_id = q.id) AS total_items, (SELECT COUNT(*) FROM quiz_answers WHERE quiz_id = q.id AND username = ?) AS my_answers FROM quizzes q WHERE q.target_username = ? OR q.target_username IS NULL OR q.target_username = '' ORDER BY q.id DESC").all(session.username, session.username);
...
app.get('/api/posts', (req, res) => {
  ...
  const posts = db.prepare('SELECT p.*, (SELECT COUNT(*) FROM likes WHERE post_id = p.id) AS like_count, (SELECT COUNT(*) FROM comments WHERE post_id = p.id) AS comment_count FROM posts p ORDER BY p.id DESC LIMIT 100').all();
...
app.post('/api/posts', (req, res) => {
  ...
  const post = db.prepare('SELECT p.*, 0 AS like_count, 0 AS comment_count, 0 AS liked FROM posts p WHERE p.id = ?').get(info.lastInsertRowid);
...
// ---------- 定期广播：每 250ms 把当前所有在线学生的最新帧推送给所有管理员 ----------
setInterval(() => {
