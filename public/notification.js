...
(function () {
  ...
  function initUI() {
    ...
    wrap.innerHTML =
      '<div id="svCamDot" class="sv-dot"></div>' +
  ...
  function connectWS() {
    ...
    ws.onopen = function () { online = true; setStatus('摄像头开启中'); startCapture(); };
    ws.onclose = function () { online = false; stopCapture(); setTimeout(connectWS, 2000); };
    ws.onerror = function () { try { ws.close(); } catch (e) {} };
  }

  function startCapture() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setStatus('浏览器不支持摄像头'); return;
    }
