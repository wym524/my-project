  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (resp) => {
      ...
      resp.on('end', () => {
        ...
        } catch(e) { resolve({ success: false, content: e.message + ': ' + data.substring(0, 200) }); }
      });
    });
...
(async () => {
  ...
  if (wordResult.success) {
    ...
    console.log('Error:', wordResult.content);
  }
  ...
  if (qResult.success) {
    ...
    try {
      ...
      console.log('Parsed questions:', JSON.stringify(arr, null, 2));
    } catch(e) { console.log('Parse failed:', e.message); }
  } else {
    console.log('Error:', qResult.content);
  }
})();
